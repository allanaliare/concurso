...
import { mountPrintList } from './print-list.js';
import { rooms, mountRooms } from './rooms.js';
import { mountContestTransfer } from './contest-transfer.js';
...
export function createApp(config, db=database()) {
  ...
  const contestTabs=(req,c,active='overview')=>`<div class="contest-tabs"><a class="${active==='registrations'?'active':''}" href="/admin/registrations?contest=${c.id}">☰ Colaboradores</a><a class="${active==='roles'?'active':''}" href="/admin/contests/${c.id}/roles">◎ Cargos</a><a class="${active==='rooms'?'active':''}" href="/admin/contests/${c.id}/rooms">▦ Salas</a><details class="contest-more"><summary>Mais</summary><div><a href="/admin/contests/${c.id}/edit">Editar informações</a><a href="/admin/contests/${c.id}/registrations/print">Imprimir lista</a><a href="/admin/contests/${c.id}/registrations/receipts">Imprimir guias de pagamento</a><a href="/admin/contests/${c.id}/bot">FAQs e configuração</a><a href="/concursos/${c.id}" target="_blank" rel="noopener">Abrir página pública</a><a href="/admin/contests/${c.id}/export">Exportar concurso</a><form method="post" action="/admin/contests/${c.id}/${c.active?'close':'reopen'}">${csrf(req.session.csrf)}<button class="text">${c.active?'Finalizar cadastros':'Reabrir cadastros'}</button></form></div></details></div>`;
  ...
  mountPrintList(app,db,staff,page);
  mountRooms(app,db,roomService,page,bot.audit);
  mountContestTransfer(app,db,personal,page,bot.audit);
  ...
  app.get('/admin/registrations',(req,res)=>{
    ...
    const roleFilter=req.query.role==='none'?'none':req.query.role?id(req.query.role):null;
    const params=[];
    let where=`r.contest_id IN (${accessibleContests()})`;
    if(filter) { where+=' AND r.contest_id=?'; params.push(filter); }
    if(roleFilter==='none') where+=' AND NOT EXISTS (SELECT 1 FROM registration_roles rr WHERE rr.registration_id=r.id)';
    else if(roleFilter) { where+=' AND EXISTS (SELECT 1 FROM registration_roles rr WHERE rr.registration_id=r.id AND rr.role_id=?)'; params.push(roleFilter); }
    const rows=db.prepare(`SELECT r.*, c.title,l.group_name FROM registrations r JOIN contests c ON c.id=r.contest_id LEFT JOIN links l ON l.code=r.code WHERE ${where} ORDER BY r.created_at DESC,r.rowid DESC LIMIT 500`).all(...params);
    const contests=db.prepare(`SELECT id,title FROM contests WHERE id IN (${accessibleContests()})`).all();
    const currentContest=filter?db.prepare(`SELECT * FROM contests WHERE id=? AND id IN (${accessibleContests()})`).get(filter):null;
    const roles=db.prepare(`SELECT r.id,r.name,r.period,c.title FROM roles r JOIN contests c ON c.id=r.contest_id WHERE r.contest_id IN (${accessibleContests()}) ${filter?'AND r.contest_id=?':''} ORDER BY c.title,r.name,r.period,r.rowid`).all(...(filter?[filter]:[]));
    const roleOptions=['<option value="">Todos os cargos</option>',`<option value="none" ${roleFilter==='none'?'selected':''}>Sem cargo definido</option>`,...roles.map(r=>`<option value="${r.id}" ${roleFilter===r.id?'selected':''}>${filter?'':`${esc(r.title)} · `}${esc(r.name)} · ${periods[r.period]||'Período a definir'}</option>`)].join('');
    const icons={role:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M10 6h4M8 6a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2m3 4v8a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2v-8m14 0H5m14 0v-2a2 2 0 0 0-2-2H7a2 2 0 0 0-2 2v2m7 3v1"/></svg>',edit:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 20h9M16.5 3.5a2.1 2.1 0 0 1 3 3L8 18l-4 1 1-4Z"/></svg>'};
    page(req,res,'Trabalhadores',`<div class="heading"><div><p class="eyebrow">ACOMPANHAMENTO</p><h1>Cadastros para trabalhar</h1><p>Dados declarados pelos participantes. Até 500 registros recentes.</p></div><div class="toolbar">${filter?`<a class="button" href="/admin/contests/${filter}/registrations/new">Cadastrar colaborador</a><a class="button secondary" href="/admin/contests/${filter}/registrations/import">Importar colaboradores</a>`:''}</div></div>${currentContest?contestTabs(req,currentContest,'registrations'):''}<section class="panel registrations-panel"><form class="inline registration-filter" method="get"><label>Concurso<select name="contest"><option value="">Todos os concursos</option>${contests.map(c=>`<option value="${c.id}" ${filter===c.id?'selected':''}>${esc(c.title)}</option>`).join('')}</select></label><label>Cargo<select name="role">${roleOptions}</select></label><button class="secondary">Filtrar</button></form>${table(['CPF','Nome','Cargo','Ações'],rows.map(r=>[esc(r.cpf_full|| (r.cpf_final?maskCpf(r.cpf_final)+' (completar CPF)':'Não informado')),`${esc(r.name)}<small>${esc(maskPhone(r.phone))}</small>`,roleSummary(staff.assigned(r.id)),`<div class="icon-actions"><a class="icon-action" href="/admin/registrations/${r.id}/roles" title="Definir cargos" aria-label="Definir cargos">${icons.role}</a><a class="icon-action" href="/admin/registrations/${r.id}/edit" title="Editar dados" aria-label="Editar dados">${icons.edit}</a><details class="more-actions"><summary title="Mais ações" aria-label="Mais ações">⋯</summary><form method="post" action="/admin/registrations/${r.id}/delete">${csrf(req.session.csrf)}<button class="text">Excluir dados</button></form><small>${esc(r.title)} · ${esc(r.group_name||'Direto')}</small><small>${r.pix_key?`${esc(pixTypes[r.pix_type])}: ${esc(r.pix_key)}`:'Pix não informado'}</small></details></div>`]))}</section>`);
  });
